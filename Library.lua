...
-- Load custom pixel font (pixelarial)
-- Uses FontFace/FontObject API (Roblox 2022+), falls back to Enum.Font.Code silently
Library.CustomFont = nil;
pcall(function()
    local fontUrl = 'https://github.com/xyznick/fonting/raw/refs/heads/main/pixelarial.TTF';
    if not isfile('pixelarial.TTF') then
        local data = game:HttpGet(fontUrl);
        if data and #data > 0 then
            writefile('pixelarial.TTF', data);
        end;
    end;
    if isfile('pixelarial.TTF') then
        local asset = getcustomasset('pixelarial.TTF');
        if asset and asset ~= '' then
            -- Font.new may not exist on all executors; guard it
            if typeof and typeof(Font) == 'table' and Font.new then
                Library.CustomFont = Font.new(asset);
            elseif Font and rawget(Font, 'new') then
                Library.CustomFont = Font.new(asset);
            end;
        end;
    end;
...
function Library:Create(Class, Properties)
    ...
    for Property, Value in next, Properties do
        if Property ~= 'Font' then  -- Font is handled below to avoid type conflicts
    ...
    if _Instance:IsA("TextLabel") or _Instance:IsA("TextBox") or _Instance:IsA("TextButton") then
        ...
        -- Apply custom font via FontFace if available, otherwise use Enum.Font fallback
        if Library.CustomFont then
            pcall(function() _Instance.FontFace = Library.CustomFont end)
        else
            pcall(function() _Instance.Font = Enum.Font.Code end)
        end
    end
...
do
    ...
    function Funcs:AddSlider(Idx, Info)
       ...
       local X = math.ceil(Library:MapValue(
        Slider.Value, Slider.Min, Slider.Max, 0, Slider.MaxSize
    ))
    ...
    HideBorderRight.Visible = not (X == Slider.MaxSize or X == 0)
        ...
        local function MakeHalf(Info)
            ...
            Library:CreateLabel({
                ...
                TextXAlignment     = Enum.TextXAlignment.Left;
                TextYAlignment     = Enum.TextYAlignment.Bottom;
                ZIndex             = 8; -- returns error? change this to 5
                Parent             = Half;
            });
