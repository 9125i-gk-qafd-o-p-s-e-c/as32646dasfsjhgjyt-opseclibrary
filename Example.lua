...
local repo = "https://raw.githubusercontent.com/liseqi/Thulib/main/"
...
-- Dual Slider
local MinSlider, MaxSlider = LeftGroupBox:AddDualSlider({
    Idx = 'MinValue',
    Text = 'Minimum',
    ...
    Callback = function(Value)
        print('[cb] Minimum changed to:', Value)
    end
}, {
    Idx = 'MaxValue',
    Text = 'Maximum',
    ...
    Callback = function(Value)
        print('[cb] Maximum changed to:', Value)
    end
})

Options.MinValue:OnChanged(function()
    print('Minimum:', Options.MinValue.Value)
end)

Options.MaxValue:OnChanged(function()
    print('Maximum:', Options.MaxValue.Value)
end)

-- Example:
Options.MinValue:SetValue(20)
Options.MaxValue:SetValue(80)
...
local WatermarkConnection = game:GetService("RunService").RenderStepped:Connect(function()
	...
	if CanDoPing then
		Library:SetWatermark(("Thulib demo | %d fps | %d ms"):format(
			math.floor(FPS),
			GetPing()
		));
	else
		Library:SetWatermark(("Thulib demo | %d fps"):format(
			math.floor(FPS)
		));
	end
end);
